...
# No control parameters needed - using direct position control
...
PLAYBACK_SPEED = 1.0  # Playback speed for video - higher values make playback slower

# No ramp durations or delays - immediate step response
...
# No ramp timing needed for immediate step response
...
# No integral control needed for direct position control
...
def plot_joint_angles():
    ...
    axes_angles[0, 0].plot(plot_data.data['time'], plot_data.data['hip_y_target_before_controller_right'], 'g--', label='Target Before Controller')
    ...
    axes_angles[2, 0].plot(plot_data.data['time'], plot_data.data['knee_target_before_controller_right'], 'g--', label='Target Before Controller')
    ...
    axes_angles[0, 1].plot(plot_data.data['time'], plot_data.data['hip_y_target_before_controller_left'], 'g--', label='Target Before Controller')
    ...
    axes_angles[2, 1].plot(plot_data.data['time'], plot_data.data['knee_target_before_controller_left'], 'g--', label='Target Before Controller')
    ...
    fig_angles.suptitle('Joint Angles vs Time\n' + 
                       f'Control Parameters: Kp={kp}, Ki=[{KI_SMALL:.1f}, {KI_MEDIUM:.1f}, {KI_LARGE:.1f}], Damping={damping}, Stiffness={stiffness}', 
                       fontsize=9, y=0.95)
...
with mujoco.viewer.launch_passive(model, data) as viewer:
    ...
    # Set all target angles immediately before simulation starts
    ...
    while viewer.is_running():
        ...
        # Set target angles immediately with no ramping
        raw_targets = {
            'right_hip': RIGHT_HIP_TARGET_ANGLE,
            'right_knee': RIGHT_KNEE_TARGET_ANGLE,
            'left_hip': LEFT_HIP_TARGET_ANGLE,
            'left_knee': LEFT_KNEE_TARGET_ANGLE
        }
        ...
        # Get current angles for plotting
